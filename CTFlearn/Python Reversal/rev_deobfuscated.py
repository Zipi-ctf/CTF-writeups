import base64
import webbrowser
import time


def mikeSwift(cre):
    sto = []
    for i in cre:
        sto.append(i+str(len(i)))  # (cre[i])"1"
        sto.append("h4ck" + i)     # "h4ck"(cre[i])
    return sto


def reverse_mikeSwift(ret):
    cre = ""
    for i in range(0, len(ret), 7):
        cre += ret[i]
    return cre


def password_prompt():
    return bytes(input("Welcome to the loading dock. What is the password?\t"), 'utf-8')


def obfuscate(input_bytes):
    obfuscated_password = base64.b64encode(input_bytes) + b"534345fdfgfgfdhty6y56yjl"
    obfuscated_password = str(obfuscated_password)[2:len(obfuscated_password)-1]
    return "florSFIUEfet4565477" + obfuscated_password


def reverse_obfuscate(ret):
    obfuscated_password = ret[len("lorSFIUEfet4565477"):]
    base64_encoded = obfuscated_password[:len(obfuscated_password) - len(b"534345fdfgfgfdhty6y56yjl") - 1]
    return base64.b64decode(base64_encoded)


def crypt(sor):
    sro = []
    fusc = "893"
    for i in range(len(sor)):
        sro.append(sor[i]+str(i))
    sro.reverse()
    for i in sro:
        fusc+=i
    return fusc


def reverse_crypt(ret):
    sor = ""
    sro = (ret[::-1])[2:]  # (reversed str(i))(sor[i]) without first 3 values
    i = 1  # index for measuring integer length in characters -> len(str(i))
    while len(sro) != 0:
        sor += sro[len(str(i))]
        sro = sro[len(str(i)) + 1:]  # +1 for sor[i] char and +len(str(i)) for i char
        i += 1
    return sor


def grant():
    print("Congartulation. Pleas Procid")
    webbrowser.open("https://ctflearn.com/index.php?action=find_problem_details&problem_id=449")


def punish():
    print("This is going to hurt.")
    while True:
        time.sleep(.1)
        #webbrowser.open("https://www.youtube.com/watch?v=O3asoGVHix8")


def main():
    # password_bytes = password_prompt()
    # sik = obfuscate(password_bytes)
    # sik = crypt(sik)
    # sik = mikeSwift(sik)
    sik = "1h4ck="
    kis = reverse_mikeSwift(sik)
    kis = reverse_crypt(kis)
    kis = reverse_obfuscate(kis)
    print(kis.decode())
main()